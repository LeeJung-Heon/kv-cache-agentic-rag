...
비용이 발생한다. 기본값은 기술 1개 × 관점 1개(Tavily 6~12회, LLM 1회)다.
    uv run python scripts/tavily_live_check.py --technology hw_01 --perspective market
녹화 축약본: tests/fixtures/tavily_responses/recorded/  (content 200자로 절단, 커밋 대상)
...
def recording_search(stamp: str, log: list, raw_dir: Path):
    def search(query, **kwargs):
        ...
        path.write_text(json.dumps({"request": request, "response": trimmed(payload)}, ensure_ascii=False, indent=2),
                        encoding="utf-8")
...
def main():
    ...
    node = make_node(RecordingAnalyst(), rules=RULES, normalize_result=normalize_result, error_result=error_result,
                     search=recording_search(stamp, log, output / "raw"))
